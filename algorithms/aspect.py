...
class Aspect(QgsProcessingAlgorithm):
    """
    ...
    def __init__(self):
        ...
        self._name = 'aspect'
        self._displayName = 'Aspect'
        self._group = None # 'Geomorphometry'
        self._groupId = None # 'Geomorphometry'
        self._shortHelp = ''
        self._helpUrl = 'https://www.whiteboxgeo.com/manual/wbw-user-manual/book/tool_help.html#aspect'
        self.params = [
            getParameterFromString('QgsProcessingParameterRasterLayer|dem1|DEM (Raster)|None|False', 'WbtAlgorithm'),
            getParameterFromString('QgsProcessingParameterNumber|z_factor2|Z factor (float)|QgsProcessingParameterNumber.Double|1.0|False|None|None', 'WbtAlgorithm'),
            getParameterFromString('QgsProcessingParameterRasterDestination|fn_output|Output (Raster)|None|False', 'WbtAlgorithm')
        ]
    ...
    def run(self, max_threads, compress_raster, params, feedback):
        # Get function parameters
        wk_dir = params['wk_dir']
        dem1 = params['dem1']
        fn_output = params['fn_output']
        z_factor2 = params['z_factor2']
        ...
        with r:
            ...
            output_raster = wbe.aspect(raster_1, z_factor2)
            wbe.write_raster(output_raster, fn_output, compress_raster)
            # sys.stdout.flush()

        feedback.pushInfo(r.stdout)
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for param in self.parameterDefinitions():
            ...
            if isinstance(param, QgsProcessingParameterRasterLayer):
                ...
                if param.dataType() == QgsProcessingParameterNumber.Integer:
                    params[param.name()] = self.parameterAsInt(parameters, param.name(), context)
                else:
                    params[param.name()] = self.parameterAsDouble(parameters, param.name(), context)
        ...
        # create a thread
        thread = Thread(target=self.run, args=(max_threads, compress_raster, params, feedback))
        # run the thread
        thread.start()
        ...
        # self.run(max_threads, compress_raster, params)
